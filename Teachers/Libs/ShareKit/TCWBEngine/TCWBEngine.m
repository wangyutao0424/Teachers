...
DEF_SINGLETION(TCWBEngine);

-(id)init{

    if (self = [super init ]) {
        
        [self initWithAppKey:WiressSDKDemoAppKey andSecret:WiressSDKDemoAppSecret andRedirectUrl:REDIRECTURI];
    }
    
    return self;
}

- (void)initWithAppKey:(NSString *)theAppKey andSecret:(NSString *)theAppSecret andRedirectUrl:(NSString *)theRedirectUrl{
...
- (void)logInWithDelegate:(id) requestDelegate  
    ...
    if ([nav.navigationBar respondsToSelector:@selector(setBackgroundImage:forBarMetrics:)]) {
        [nav.navigationBar setBackgroundImage:[UIImage imageNamed:@"_nav.png"] forBarMetrics:UIBarMetricsDefault];
	}
