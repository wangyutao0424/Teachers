...
- (void)startAuthorize
{
    ...
    NSArray *cookieAry = [cookieJar cookiesForURL: [NSURL URLWithString:urlString]];
    ...
    UINavigationController * _navC = [[UINavigationController alloc] initWithRootViewController:webView];
    if ([_navC.navigationBar respondsToSelector:@selector(setBackgroundImage:forBarMetrics:)]) {
        [_navC.navigationBar setBackgroundImage:[UIImage imageNamed:@"_nav.png"] forBarMetrics:UIBarMetricsDefault];
    }

    _navC.navigationBar.backgroundColor = [UIColor blackColor];
    [self.rootViewController presentModalViewController:_navC animated:YES];
    [_navC release];
...
- (void)request:(WBRequest *)theRequest didFinishLoadingWithResult:(id)result requestType:(WBRequestType)type
{
    BOOL success = NO;
